...
class Player9(Player):
    ...
    def _do_move(
        self, x: float, y: float, reason: str = "manual", allow_noop: bool = False
    ) -> Move | None:
        """
        ...
        try:
            print(
                f"Helper {getattr(self, 'id', None)} MOVE reason={reason} from={self.position} to=({x:.2f},{y:.2f})"
            )
        except Exception:
            pass
        return Move(x, y)
    ...
    def check_surroundings(self, snapshot: HelperSurroundingsSnapshot) -> int:
        ...
        if self.kind == Kind.Noah:
            ...
            if (
                getattr(snapshot, "is_raining", False)
                and getattr(self, "rain_start_turn", None) is None
            ):
                try:
                    self.rain_start_turn = snapshot.time_elapsed
                except Exception:
    ...
    def get_action(self, messages: list[Message]) -> Action | None:
        ...
        if hasattr(self, "time_remaining"):
            ...
            if dist_to_ark > 40:
                act = self._do_move(
                    *self.move_towards(*self.ark_position),
                    reason="return_due_to_time_remaining",
                    allow_noop=True,
                )
                if act is not None:
                    return act
        ...
        if (int(self.position[0]), int(self.position[1])) == tuple(
            self.ark_position
        ) and flock_size > 0:
            act = self._do_move(
                *self.move_towards(*self.ark_position),
                reason="offload_at_ark",
                allow_noop=True,
            )
            if act is not None:
                return act
        ...
        if flock_size > 1:
            ...
            if getattr(self, "local_collects", 0) < getattr(
                ...
                if nearby:
                    ...
                    act = self._do_move(
                        *self.move_towards(nx + 0.5, ny + 0.5),
                        reason="move_to_nearby_collect",
                    )
                    if act is not None:
                        return act
        ...
        if at_ark:
            ...
            if best is not None:
                ...
                tx, ty = best
                act = self._do_move(
                    *self.move_towards(tx + 0.5, ty + 0.5),
                    reason="move_to_adjacent_ark_cell",
                )
                if act is not None:
                    return act
        ...
        if flock_size > 0:
            ...
            if comp:
                ...
                act = self._do_move(
                    *self.move_towards(tx + 0.5, ty + 0.5), reason="move_to_complement"
                )
                if act is not None:
                    return act
        ...
        if getattr(self, "role", "sweeper") == "cluster":
            ...
            if best:
                ...
                act = self._do_move(
                    *self.move_towards(best[0] + 0.5, best[1] + 0.5),
                    reason="move_to_cluster_target",
                )
                if act is not None:
                    return act
        ...
        if getattr(self, "role", "sweeper") == "ark_runner":
            ...
            if best:
                ...
                act = self._do_move(
                    *self.move_towards(best[0] + 0.5, best[1] + 0.5),
                    reason="move_to_ark_runner_target",
                )
                if act is not None:
                    return act
        ...
        if corridor_target and not self.is_raining:
            ...
            act = self._do_move(
                *self.move_towards(tx + 0.5, ty + 0.5), reason="move_to_corridor_target"
            )
            if act is not None:
                return act
        ...
        if best_animal_pos:
            ...
            act = self._do_move(
                *self.move_towards(bx + 0.5, by + 0.5), reason="move_to_chase_target"
            )
            if act is not None:
                return act
        ...
        try:
            print(
                f"Helper {getattr(self, 'id', None)} MOVE reason=sweep from={self.position} to=({new_x:.2f},{new_y:.2f})"
            )
        except Exception:
            pass
